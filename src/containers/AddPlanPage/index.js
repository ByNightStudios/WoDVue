...
import AddPlanManagerFile from './dataManager';
import hasPermission from '../../hoc/hasPermission';

import styles from './add-plan-page.scss';

const AddPlanManager = new AddPlanManagerFile();
...
class AddPlanPage extends React.Component {
  ...
  componentDidMount() {
    document.title = 'Emoha Admin | Add a Plan';
    this.getPlanCategories();
  }

  getPlanCategories = () => {
    this._startPageLoader();
    AddPlanManager.getPlanCategories()
      .then((res) => {
        this.setState({ categories: res.data, loader: false });
      })
      .catch((error) => {
  ...
  addPlanHandler = () => {
    const formValidity = AddPlanManager.addPlanValidator(this.state.formData);

    if (!formValidity.status) {
      return this.openNotification('Error', formValidity.message, 0);
    } else {
      this._startPageLoader();
      AddPlanManager.addPlan(this.state.formData)
        .then((res) => {
          this._stopPageLoader();
          this.openNotification('Success', 'Plan Added Successfully.', 1);
...
export default hasPermission(
  requireAuth(connect(mapStateToProps, {})(AddPlanPage))
);
